...
mono_marshal_get_delegate_begin_invoke (MonoMethod *method)
{
	 ...
	 */
	if (ctx) {
		cache = get_cache (&orig_method->klass->generic_class->owner->delegate_begin_invoke_cache, mono_aligned_addr_hash, NULL);
...
mono_marshal_get_delegate_end_invoke (MonoMethod *method)
{
	 ...
	 */
	if (ctx) {
		cache = get_cache (&orig_method->klass->generic_class->owner->delegate_end_invoke_cache, mono_aligned_addr_hash, NULL);
		res = check_generic_delegate_wrapper_cache (cache, orig_method, method, ctx);
		if (res)
			return res;
	} else {
...
mono_marshal_get_delegate_invoke_internal (MonoMethod *method, gboolean callvirt, gboolean static_method_with_first_arg_bound, MonoMethod *target_method)
{
	 ...
	 */
	if (ctx) {
		cache = get_cache (&orig_method->klass->generic_class->owner->delegate_invoke_cache, mono_aligned_addr_hash, NULL);
		res = check_generic_delegate_wrapper_cache (cache, orig_method, method, ctx);
		if (res)
			return res;
...
mono_marshal_get_synchronized_wrapper (MonoMethod *method)
{
	 ...
	 */
	if (ctx) {
		cache = get_cache (&orig_method->klass->generic_class->owner->synchronized_cache, mono_aligned_addr_hash, NULL);
